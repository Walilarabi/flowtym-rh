...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 3. Vérifier que l'appelant est direction/admin pour cet hôtel
    ...
    if (!ch || !['direction','admin_hotel'].includes(ch.role))
      return json({error:'Droits direction ou admin_hotel requis pour cet hotel'},403);

    // 4. Inviter (ou ignorer si déjà inscrit)
    ...
    if (invErr) {
      ...
      // Déjà inscrit — retrouver l'auth_id
    ...
    // 5. Upsert public.users + user_hotels via fonction SQL atomique
