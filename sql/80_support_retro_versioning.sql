...
--     (support_contract_v1) ; le détail par catégorie (colonnes, contraintes,
--     index, séquence, fonctions, triggers, policies, grants, commentaires,
--     RLS/propriétaire) reste dans le diagnostic, jamais un moteur de
--     décision indépendant par catégorie.
--
-- CE FICHIER NE CONTIENT AUCUN BEGIN / COMMIT / ROLLBACK — invocation :
--   Production      : appliqué par l'outillage de déploiement habituel du
--                      projet (transaction gérée par l'appelant).
--   Reconstruction  : \i sql/80_support_retro_versioning.sql à l'intérieur
--                      d'un BEGIN...ROLLBACK — voir
--                      sql/tests/support_retro_versioning.sql (le fichier de
--                      test est seul responsable de BEGIN/ROLLBACK).
--   Base jetable    : psql --set ON_ERROR_STOP=1 --single-transaction \
...
\gset support_tickets_

\if :support_tickets_all_absent

\echo '[support_tickets] domaine totalement absent — création complète (support_contract_v1).'
...
CREATE INDEX idx_support_tickets_hotel ON public.support_tickets USING btree (hotel_id, created_at DESC);
CREATE INDEX idx_support_tickets_status ON public.support_tickets USING btree (hotel_id, status);
...
CREATE TRIGGER trg_support_ticket_number BEFORE INSERT ON public.support_tickets FOR EACH ROW EXECUTE FUNCTION public.support_set_ticket_number();
CREATE TRIGGER trg_support_updated_at BEFORE UPDATE ON public.support_tickets FOR EACH ROW EXECUTE FUNCTION public.support_set_updated_at();

ALTER TABLE public.support_tickets ENABLE ROW LEVEL SECURITY;

CREATE POLICY platform_admin_read_tickets ON public.support_tickets FOR SELECT TO public USING (is_platform_admin());
CREATE POLICY platform_admin_update_tickets ON public.support_tickets FOR UPDATE TO public USING (is_platform_admin()) WITH CHECK (is_platform_admin());
CREATE POLICY support_select ON public.support_tickets FOR SELECT TO public USING (hotel_id IN (SELECT user_hotels.hotel_id FROM public.user_hotels WHERE user_hotels.user_id = auth.uid()));
CREATE POLICY support_update ON public.support_tickets FOR UPDATE TO public USING (hotel_id IN (SELECT user_hotels.hotel_id FROM public.user_hotels WHERE user_hotels.user_id = auth.uid()));
CREATE POLICY support_insert ON public.support_tickets FOR INSERT TO public WITH CHECK (hotel_id IN (SELECT user_hotels.hotel_id FROM public.user_hotels WHERE user_hotels.user_id = auth.uid()));

GRANT INSERT, SELECT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON public.support_tickets TO anon, authenticated, service_role, postgres;
GRANT USAGE ON SEQUENCE public.support_ticket_seq TO anon, authenticated, service_role, postgres;
GRANT EXECUTE ON FUNCTION public.support_set_ticket_number() TO anon, authenticated, service_role, postgres;
GRANT EXECUTE ON FUNCTION public.support_set_updated_at() TO anon, authenticated, service_role, postgres;

COMMENT ON COLUMN public.support_tickets.risk_score IS 'Computed risk level: faible | moyen | eleve | critique';
COMMENT ON COLUMN public.support_tickets.diagnostic_details IS 'Structured diagnostic: externalCheckResult, riskAnalysis, correctionStrategy, affectedModules, testChecklist';

\echo '[support_tickets] création complète terminée.'

\elif :support_tickets_all_present

DO $check_support_tickets$
DECLARE
...
BEGIN
  ...
  -- NB: last_value volontairement exclu du fingerprint (non transactionnel, varie légitimement).
...
\if :help_articles_all_absent

\echo '[help_articles] domaine totalement absent — création complète (support_contract_v1).'
...
COMMENT ON TABLE public.help_articles IS 'Documentation Flowtym par module. Lecture pour tout utilisateur authentifié (articles publiés). Écriture pour super_admin / support_agent.';

CREATE INDEX idx_help_articles_module ON public.help_articles USING btree (module, sort_order);
CREATE INDEX idx_help_articles_published ON public.help_articles USING btree (is_published, module);
...
CREATE TRIGGER trg_help_articles_updated BEFORE UPDATE ON public.help_articles FOR EACH ROW EXECUTE FUNCTION public.trg_help_articles_updated_at();

ALTER TABLE public.help_articles ENABLE ROW LEVEL SECURITY;

CREATE POLICY help_articles_select ON public.help_articles FOR SELECT TO public USING ((is_published = true) OR is_platform_admin());
CREATE POLICY help_articles_write ON public.help_articles FOR ALL TO public USING (platform_admin_role() = ANY (ARRAY['super_admin'::text,'support_agent'::text])) WITH CHECK (platform_admin_role() = ANY (ARRAY['super_admin'::text,'support_agent'::text]));

GRANT INSERT, SELECT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON public.help_articles TO anon, authenticated, service_role, postgres;
GRANT EXECUTE ON FUNCTION public.trg_help_articles_updated_at() TO anon, authenticated, service_role, postgres;

\echo '[help_articles] création complète terminée.'

\elif :help_articles_all_present

DO $check_help_articles$
DECLARE
